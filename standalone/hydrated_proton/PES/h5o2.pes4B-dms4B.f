...
! H  3.201587564645578 0.4544810799466731 1.498136372134866
!  
!  prepot_z() should be called before first calling calcpot_z(V,xx)
...
!
!  predip_z() should be called before first calling calcdip_z(dip,xx)
!    xx(7,3) is cartesian array containing O O H H H H H coor, in bohr
!    returned dip(1),dip(2),dip(3) are the x-, y-, z- dipole components
!              relative to the center-of-mass of H5O2+, in a.u.
!
!  For D5O2+ calculations, you may need to change calcdip_z() to user D5O2+ center-of-mass frame
!
!
...
!***************************************************************
        ...
        subroutine calcpot_z(V,cart_in)
        ...
        double precision cart0(7,3),cart1(7,3)
        integer i,j,k,l,i1,j1,k1,l1,i2,j2,k2,l2


        double precision rvec(0:3),d0(0:6,0:6),r0(0:6,0:6),
     $  vec(0:ms+3*mr-1)
        ...
        call getvec_z (ms, mr, xnuc(0:2,0:6), dc0, dw0, vec)
...
!        V = V + 153.0119260317857d0
        ...
        return
        end subroutine calcpot_z
!********************************************************
        subroutine getvec_z (ms, mr, xn, dc0, dw0, vec)                                
        implicit none                                                                
...
!-----------------------------------------------------------------------     
        vec=0                                                                      
        call getr0_z (nk, xn, r0)                                                      
        call getd0_z (nk, r0, dc0, dw0, d0)                                            
        call getv_x5y2_z (ms, d0, vec(0:ms-1))                                         
        call getrvec_z (4, r0, rvec)                                                   
        do l=0, mr-1                                                               
         do k=0, 2                                                                 
        ...
        subroutine getv_x5y2_z (m, d, vec)                                             
        implicit none                                                                
        integer, parameter :: wp=selected_real_kind(12,300)                          
...
!! We haven't incorporated the secondaries at degree 7                       
     ...
     $     vs(0:ns5-1), ws(0:ns6-1), w7s(0:ns7-1)                                     
        integer mdeg, i, j, k, l, i0, i1, i2, i3, i4, j0, j1                         
...
! Test for compatibility                                                     
        if (.not.(m.eq.1.or.m.eq.4)) then                                            
         stop 'getrvec_z - wrong dimension'                                            
        endif                                                                        
! Computation                                                                
        x=0                                                                        
        do i=0, nk-1                                                               
...
! set vec                                                                    
        ...
        subroutine predip_z(filename)

!        implicit none
        implicit double precision (a-h,o-z)
        implicit integer (i-n)

        double precision dip(3), cart_in(7,3), v0, dip_coef(0:3843)
...
!*******************************************************************
        ...
        subroutine calcdip_z(dip,chg,cart_in)

        implicit none

        double precision dip(3), cart_in(7,3), v0, dip_coef(0:3843)
        double precision dip_dc0(0:6,0:6), dip_dw0(0:6,0:6)
        double precision chg(7),chg_ord(0:6),chgb(0:6,0:3843)
...
!! shift to h5o2 C.O.M. frame (O2H5)
     ...
     $  xn(0:2,4)))/(15.9949146d0*2+1.007825d0*5)
        ...
        call getdvec_z (ma, mb, xnuc(0:2,0:6),dip_dc0,dip_dw0,vec,chgb)
        ...
        chg_ord=0.d0
        do i=0,6
           chg_ord(i)=dot_product(chgb(i,:),dip_coef)
        end do
        chg(1:2)=chg_ord(5:6)
        chg(3:7)=chg_ord(0:4)
...
!        write(2002,*)dip
        ...
        return
        end subroutine calcdip_z
!********************************************************
!*********************************************************************                     
        subroutine getscale_z (n, xnuc, dc0, dw0)                                      
        implicit none                                                                
        integer, parameter :: wp=selected_real_kind(12,300)                          
! Version for X5Y2                                                           
        integer n, nk                                                                
...
! evaluate dc0                                                               
        dt0 = 0 ; dc1 = 0 ; dw1 = 1                                                  
        do ip = 0, n-1                                                               
         call getr0_z (nk, xnuc(0:2,0:nk-1,ip), r0)                                    
         call getd0_z (nk, r0, dc1, dw1, d0)                                           
         dt0 = dt0+d0/n                                                              
        enddo                                                                        
        do i = 0, nk-1                                                               
         dc0(i,i) = 0                                                                
...
! evaluate dw0                                                               
     ...
     $       (xn(2,j)-xn(2,i))**2)                                                    
        ...
        subroutine getd0_z (nk, r0, dc0, dw0, d0)                                      
        implicit none                                                                
        integer, parameter :: wp=selected_real_kind(12,300)                          
        integer nk                                                                   
...
!          d0(i,j) = (dlog(r0(i,j))-dc0(i,j))/dw0(i,j)                                 
        ...
        subroutine getdvec_z (ma, mb, xn, dc0, dw0, vec,chg)                               
        implicit none                                                                
        integer, parameter :: wp=selected_real_kind(12,300)                          
! version for H5O2(+)                                                        
        integer nk, ma, mb                                                           
...
! vector factor xn(0:2,6) (species Y)                                        
        ...
        subroutine getv_x4y2z_z (m, d, vec)                                            
        implicit none                                                                
        integer, parameter :: wp=selected_real_kind(12,300)                          
...
!! Note, we have included only 86 of the 199 secondaries at degree 5.        
     ...
     $  us(0:ns4-1), vs(0:ns5-1), ws(0:ns6-1), w7s(0:ns7-1)                                     
        integer mdeg, i, j, k, l, i0, i1, i2, j0, j1, k0                             
        double precision d2(0:nk-1,0:nk-1), d3(0:nk-1,0:nk-1), 
     $  d4(0:nk-1,0:nk-1), d5(0:nk-1,0:nk-1), d6(0:nk-1,0:nk-1), 
     $  d7(0:nk-1,0:nk-1)                    
        double precision t0                                                            
...
!! should be vec(2426:2624) = vs(0:198)                                      
        ...
        subroutine getv_x5yz_z (m, d, vec)                                             
        implicit none                                                                
        integer, parameter :: wp=selected_real_kind(12,300)                          
        integer, parameter :: nk=7                                                   
        integer m                                                                    
