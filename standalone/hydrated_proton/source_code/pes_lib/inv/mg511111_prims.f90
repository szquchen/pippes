...
if (size(r,1).ne.nk.or.size(r,2).ne.nk.or.size(u).ne.nr) then
 stop 'mg511111_prims: bad dimensions'
endif
...
u5(0) = x(m2+3*m+1)
u6(0) = x(m2+3*m+2)
call sym5_prims (x(m2+3*m+3:m2+4*m+2), u7)
u8(0) = x(m2+4*m+3)
u9(0) = x(m2+4*m+4)
u10(0) = x(m2+4*m+5)
call sym5_prims (x(m2+4*m+6:m2+5*m+5), u11)
u12(0) = x(m2+5*m+6)
u13(0) = x(m2+5*m+7)
u14(0) = x(m2+5*m+8)
u15(0:0) = x(m2+5*m+9:nr-1)
u = (/ u0(0), u1(0), u2(0), u3(0), u4(0), u5(0), u6(0), u7(0), &
  ...
  u0(5), u0(6), u1(3), u2(3), u4(3), u7(3), u11(3), &
  u0(7), u0(8), u1(4), u2(4), u4(4), u7(4), u11(4), &
  u0(9) /)
return
END SUBROUTINE mg511111_prims
