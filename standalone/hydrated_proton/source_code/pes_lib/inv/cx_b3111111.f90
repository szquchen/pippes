SUBROUTINE cx_b3111111 (nki, ik, pc, r, w)
...
real (kind=wp) :: t0, r0(0:mg3111111_nk-1,0:mg3111111_nk-1), &
  y0(0:mg3111111_nk-1,0:mg3111111_nk-1), &
  w0(0:cx_dim(mg3111111_nb,pc%dg)-1), w1(0:cx_dim(mg3111111_nb,pc%dg)-1)
if (any(size(nki).lt.ik)) then
 stop 'cx_b3111111: bad nki, ik'
else if (size(ik).ne.mg3111111_nkk) then
 stop 'cx_b3111111: bad dimension ik'
else if (size(r,1).ne.sum(nki).or.size(r,2).ne.sum(nki)) then
 stop 'cx_b3111111: bad dimension nki, r'
else if (size(w).ne.cx_dim(mg3111111_nb,pc%dg)) then
 stop 'cx_b3111111: bad dimension w'
endif
w1 = 0
if (0.le.pc%dg) then
        ...
        do i1 = sum(nki(0:ik(0)-1))+1, i2-1
         do i0 = sum(nki(0:ik(0)-1)), i1-1
          ...
          if (t0.ne.0) then
           ...
           call mg3111111_base (pc%dg, y0, w0)
           w1 = w1+w0*t0
          endif
         enddo
        enddo
