...
if (size(r,1).ne.nk.or.size(r,2).ne.nk.or.size(u).ne.nr) then
 stop 'mg33111_prims: bad dimensions'
endif
...
u7(0) = x(ib0+m+n)
call sym3_prims (x(ib0+m+n+1:ib0+2*m+n), u8)
call sym3_prims (x(ib0+2*m+n+1:ib0+2*m+2*n), u9)
