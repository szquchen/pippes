SUBROUTINE cx_b721 (nki, ik, pc, r, w)
...
real (kind=wp) :: t0, r0(0:mg721_nk-1,0:mg721_nk-1), &
  ...
  w0(0:cx_dim(mg721_nb,pc%dg)-1), w1(0:cx_dim(mg721_nb,pc%dg)-1)
if (any(size(nki).lt.ik)) then
 stop 'cx_b721: bad nki, ik'
else if (size(ik).ne.mg721_nkk) then
 stop 'cx_b721: bad dimension ik'
else if (size(r,1).ne.sum(nki).or.size(r,2).ne.sum(nki)) then
 stop 'cx_b721: bad dimension nki, r'
else if (size(w).ne.cx_dim(mg721_nb,pc%dg)) then
 stop 'cx_b721: bad dimension w'
endif
w1 = 0
if (0.le.pc%dg) then
