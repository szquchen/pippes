SUBROUTINE mg31111111_prims (r, u)
...
real (kind=wp) :: x(0:nr-1), u0(0:m2-1), u1(0:m-1), u2(0:m-1), u3(0:0), &
  ...
  u22(0:m-1), u23(0:0), u24(0:0), u25(0:0), u26(0:0), u27(0:0), u28(0:0)
if (size(r,1).ne.nk.or.size(r,2).ne.nk.or.size(u).ne.nr) then
 stop 'mg31111111_prims: bad dimensions'
endif
...
u9(0) = x(m2+4*m+4)
u10(0) = x(m2+4*m+5)
call sym3_prims (x(m2+4*m+6:m2+5*m+5), u11)
u12(0) = x(m2+5*m+6)
u13(0) = x(m2+5*m+7)
u14(0) = x(m2+5*m+8)
u15(0) = x(m2+5*m+9)
call sym3_prims (x(m2+5*m+10:m2+6*m+9), u16)
u17(0) = x(m2+6*m+10)
u18(0) = x(m2+6*m+11)
u19(0) = x(m2+6*m+12)
u20(0) = x(m2+6*m+13)
u21(0) = x(m2+6*m+14)
call sym3_prims (x(m2+6*m+15:m2+7*m+14), u22)
u23(0) = x(m2+7*m+15)
u24(0) = x(m2+7*m+16)
u25(0) = x(m2+7*m+17)
u26(0) = x(m2+7*m+18)
...
u = (/ u0(0), u1(0), u2(0), u3(0), u4(0), u5(0), u6(0), u7(0), &
  ...
  u0(1), u1(1), u2(1), u4(1), u7(1), u11(1), u16(1), u22(1), &
  u0(2), u1(2), u2(2), u4(2), u7(2), u11(2), u16(2), u22(2) /)
return
END SUBROUTINE mg31111111_prims
