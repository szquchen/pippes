SUBROUTINE cx_b73 (nki, ik, pc, r, w)
...
real (kind=wp) :: t0, r0(0:mg73_nk-1,0:mg73_nk-1), &
  ...
  w0(0:cx_dim(mg73_nb,pc%dg)-1), w1(0:cx_dim(mg73_nb,pc%dg)-1)
if (any(size(nki).lt.ik)) then
 stop 'cx_b73: bad nki, ik'
else if (size(ik).ne.mg73_nkk) then
 stop 'cx_b73: bad dimension ik'
else if (size(r,1).ne.sum(nki).or.size(r,2).ne.sum(nki)) then
 stop 'cx_b73: bad dimension nki, r'
else if (size(w).ne.cx_dim(mg73_nb,pc%dg)) then
 stop 'cx_b73: bad dimension w'
endif
w1 = 0
if (0.le.pc%dg) then
