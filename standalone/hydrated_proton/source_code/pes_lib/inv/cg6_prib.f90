SUBROUTINE cg6_prib (mxd, x, w)
integer, intent (in) :: mxd
real (kind=wp), intent (in) :: x(0:)
real (kind=wp), intent (out) :: w(0:cg6_npb(mxd)-1)
!-----------------------------------------------------------------------
real (kind=wp) :: u(0:nr-1)
if (size(x).ne.nr) then
 stop 'cg6_prib: bad size x'
!! else if (size(w).ne.cg6_npb(mxd)) then
!!  stop 'cg6_prib: bad size w'
endif
call cg6_prims (x, u)
