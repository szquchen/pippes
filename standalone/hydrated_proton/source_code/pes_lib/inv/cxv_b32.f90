SUBROUTINE cxv_b32 (nki, ik, pc, r, w)
...
real (kind=wp) :: t0, r0(0:mg32_nk-1,0:mg32_nk-1), &
  ...
  w0(0:mg32_nk-1,0:cx_dim(mgv32_nb,pc%dg)-1), &
  w1(0:size(w,1)-1,0:cx_dim(mgv32_nb,pc%dg)-1)
if (any(size(nki).lt.ik)) then
 stop 'cxv_b32: bad nki, ik'
else if (size(ik).ne.mg32_nkk) then
 stop 'cxv_b32: bad dimension ik'
else if (size(r,1).ne.sum(nki).or.size(r,2).ne.sum(nki)) then
 stop 'cxv_b32: bad dimension nki, r'
else if (size(w,1).ne.sum(nki)) then
 stop 'cxv_b32: bad dimension w (1)'
else if (size(w,2).ne.cx_dim(mgv32_nb,pc%dg)) then
 stop 'cxv_b32: bad dimension w (2)'
endif
w1 = 0
if (0.le.pc%dg) then
    ...
    do i1 = sum(nki(0:ik(0)-1))+1, i2-1
     do i0 = sum(nki(0:ik(0)-1)), i1-1
      ...
      if (t0.ne.0) then
       ...
       call mgv32_base (pc%dg, y0, w0)
       w1((/i0,i1,i2,j0,j1/),:) = w1((/i0,i1,i2,j0,j1/),:)+w0*t0
      endif
     enddo
    enddo
   enddo
