SUBROUTINE mg7_prib (mxd, u, w)
integer, intent (in) :: mxd
real (kind=wp), intent (in) :: u(0:)
real (kind=wp), intent (out) :: w(0:mg7_npb(mxd)-1)
!-----------------------------------------------------------------------
if (size(u).ne.nr) then
 stop 'mg7_prib: bad size u'
!! else if (size(w).ne.mg7_npb(mxd)) then
!!  stop 'mg7_prib: bad size w'
endif
