...
integer, parameter :: mxd=MXD, nk=MG()_nk, nr=MG()_nr, &
  nkj(0:size(MG()_nkj)-1)=MG()_nkj, m=MG()_nb(mxd), mpb=MG()_npb(mxd), &
  mvec=MGV()_nb(mxd), ntst=2*m, lwork=20*ntst
...
real (kind=wp) :: t0, x(0:nr-1), y(0:nr-1), z(0:nr-1), &
  u(0:nr-1), u0(0:nr-1), v(0:MG()_nsc(mxd)-1), &
  v0(0:MG()_nsc(mxd)-1), r(0:nk-1,0:nk-1), r1(0:nk-1,0:nk-1)
...
if (product(factorial(nkj)).ne.MG()_ngrp) then
 stop 'test - nkj, ngrp parameter fault'
endif
do k = 0, size(MG()_nb)-1
 if (MG()_npr(k).ne.sum(MG()_dnpr(0:k)).or. &
   MG()_nsc(k).ne.sum(MG()_dnsc(0:k)).or. &
   MG()_nb(k).ne.sum(MG()_dnb(0:k))) then
...
r = 2
call MG()_prims (r, u)
do k = 1, size(MG()_nb)-1
 do j = MG()_npr(k-1), MG()_npr(k)-1
...
r = 1
call MG()_secs (mxd, r, v)
b0 = .false.
do j = 0, MG()_nsc(mxd)-1
...
do i = 0, 1
 call random_number (r)
 r = r+transpose(r)-1
 call MG()_secs (mxd, r, v0)
 do j = 0, mgx_ngen(nkj)-1
 b0 = .false.
  call mgx_gens2d (nkj, j, iord)
  r1 = r(iord,iord)
  call MG()_secs (mxd, r1, v)
  t0 = maxval(abs(v-v0))
...
do i = 0, 1
 call random_number (r)
 r = r+transpose(r)-1
 call MGV()_base (mxd, r, wvec)
 do j = 0, mgx_ngen(nkj)-1
  call mgx_gens2d (nkj, j, iord)
  r1 = r(iord,iord)
  call MGV()_base (mxd, r1, w1vec)
...
if (m.lt.mxm_test) then
 ...
 do i = 0, ntst-1
  call random_number (r)
  r = r+transpose(r)-1
  call MG()_base (mxd, r, mat(i,0:m-1))
 enddo
 do i = 0, m-1
