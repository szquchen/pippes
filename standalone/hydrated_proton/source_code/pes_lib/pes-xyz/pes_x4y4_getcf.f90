SUBROUTINE pes_x4y4_getcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x4y4_nki)) then
 stop 'pes_x4y4_getcf: bad dimension'
else if (any(nki.ne.pes_x4y4_nki)) then
 stop 'pes_x4y4_getcf: bad nki'
endif
if (all(nkj.eq.(/1,0/))) then
 ...
 call pes_x4y4_add (pc, cf)
 call pes_write (iun, 'pcf-x4y4', &
   pes_x4y4_pc, pes_x4y4_cf)
else
 stop 'pes_x4y4_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x4y4_getcf
