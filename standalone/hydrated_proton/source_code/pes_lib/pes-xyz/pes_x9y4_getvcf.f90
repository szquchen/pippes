SUBROUTINE pes_x9y4_getvcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x9y4_nki)) then
 stop 'pes_x9y4_getvcf: bad dimension'
else if (any(nki.ne.pes_x9y4_nki)) then
 stop 'pes_x9y4_getvcf: bad nki'
endif
...
!! call pes_write (iun, 'vpcf-x7y3', pes_x7y3_vpc, pes_x7y3_vcf)
 ...
 stop 'pes_x9y4_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x9y4_getvcf
