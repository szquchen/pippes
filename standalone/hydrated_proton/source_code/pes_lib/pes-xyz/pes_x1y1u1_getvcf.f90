SUBROUTINE pes_x1y1u1_getvcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x1y1u1_nki)) then
 stop 'pes_x1y1u1_getvcf: bad dimension'
else if (any(nki.ne.pes_x1y1u1_nki)) then
 stop 'pes_x1y1u1_getvcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_write (iun, 'vpcf-x1y1u1', &
   pes_x1y1u1_vpc, pes_x1y1u1_vcf)
else
 stop 'pes_x1y1u1_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x1y1u1_getvcf
