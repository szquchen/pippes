SUBROUTINE pes_x5y4_getvcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x5y4_nki)) then
 stop 'pes_x5y4_getvcf: bad dimension'
else if (any(nki.ne.pes_x5y4_nki)) then
 stop 'pes_x5y4_getvcf: bad nki'
endif
if (all(nkj.eq.(/1,0/))) then
 ...
 call pes_x5y4_vadd (pc, cf)
 call pes_write (iun, 'vpcf-x5y4', &
   pes_x5y4_vpc, pes_x5y4_vcf)
else
 stop 'pes_x5y4_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x5y4_getvcf
