FUNCTION pes_x4y1_vfun (xn) RESULT (f)
! Vector function for generic X4Y1
real (kind=wp), intent (in) :: xn(0:,0:)
real (kind=wp) :: f(0:size(xn,2)-1)
!-----------------------------------------------------------------------
real (kind=wp) :: r(0:pes_x4y1_nk-1,0:pes_x4y1_nk-1)
integer, parameter :: nki(0:1)=pes_x4y1_nki
call pes_dists (xn, r)
