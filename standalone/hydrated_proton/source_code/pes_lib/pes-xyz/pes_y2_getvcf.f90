SUBROUTINE pes_y2_getvcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_y2_nki)) then
 stop 'pes_y2_getvcf: bad dimension'
else if (any(nki.ne.pes_y2_nki)) then
 stop 'pes_y2_getvcf: bad nki'
endif
if (all(nkj.eq.(/1/))) then
 ...
 stop 'pes_y2_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_y2_getvcf
