SUBROUTINE pes_x4y2z1u1_getcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x4y2z1u1_nki)) then
 stop 'pes_x4y2z1u1_getcf: bad dimension'
else if (any(nki.ne.pes_x4y2z1u1_nki)) then
 stop 'pes_x4y2z1u1_getcf: bad nki'
endif
...
else if (all(nkj.eq.(/0,2,0,0/))) then
...
else if (all(nkj.eq.(/1,2,0,0/))) then
...
else if (all(nkj.eq.(/0,2,1,0/))) then
 call pes_y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-y2z1', pes_y2z1_pc, pes_y2z1_cf)
else if (all(nkj.eq.(/2,0,0,1/))) then
 call pes_x2u1_add (pc, cf)
 call pes_write (iun, 'pcf-x2u1', pes_x2u1_pc, pes_x2u1_cf)
else if (all(nkj.eq.(/1,1,0,1/))) then
 call pes_x1y1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y1u1', pes_x1y1u1_pc, pes_x1y1u1_cf)
else if (all(nkj.eq.(/0,2,0,1/))) then
 call pes_y2u1_add (pc, cf)
 call pes_write (iun, 'pcf-y2u1', pes_y2u1_pc, pes_y2u1_cf)
else if (all(nkj.eq.(/1,0,1,1/))) then
 call pes_x1z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x1z1u1', pes_x1z1u1_pc, pes_x1z1u1_cf)
else if (all(nkj.eq.(/0,1,1,1/))) then
 call pes_y1z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-y1z1u1', pes_y1z1u1_pc, pes_y1z1u1_cf)
...
else if (all(nkj.eq.(/2,2,0,0/))) then
...
else if (all(nkj.eq.(/1,2,1,0/))) then
 call pes_x1y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y2z1', pes_x1y2z1_pc, pes_x1y2z1_cf)
else if (all(nkj.eq.(/3,0,0,1/))) then
 call pes_x3u1_add (pc, cf)
 call pes_write (iun, 'pcf-x3u1', pes_x3u1_pc, pes_x3u1_cf)
else if (all(nkj.eq.(/2,1,0,1/))) then
 call pes_x2y1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y1u1', pes_x2y1u1_pc, pes_x2y1u1_cf)
else if (all(nkj.eq.(/1,2,0,1/))) then
 call pes_x1y2u1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y2u1', pes_x1y2u1_pc, pes_x1y2u1_cf)
else if (all(nkj.eq.(/2,0,1,1/))) then
 call pes_x2z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x2z1u1', pes_x2z1u1_pc, pes_x2z1u1_cf)
else if (all(nkj.eq.(/1,1,1,1/))) then
 call pes_x1y1z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y1z1u1', pes_x1y1z1u1_pc, pes_x1y1z1u1_cf)
else if (all(nkj.eq.(/0,2,1,1/))) then
 call pes_y2z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-y2z1u1', pes_y2z1u1_pc, pes_y2z1u1_cf)
else if (all(nkj.eq.(/4,1,0,0/))) then
 call pes_x4y1_add (pc, cf)
 call pes_write (iun, 'pcf-x4y1', pes_x4y1_pc, pes_x4y1_cf)
else if (all(nkj.eq.(/3,2,0,0/))) then
...
else if (all(nkj.eq.(/2,2,1,0/))) then
 call pes_x2y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2z1', pes_x2y2z1_pc, pes_x2y2z1_cf)
else if (all(nkj.eq.(/4,0,0,1/))) then
 call pes_x4u1_add (pc, cf)
 call pes_write (iun, 'pcf-x4u1', pes_x4u1_pc, pes_x4u1_cf)
else if (all(nkj.eq.(/3,1,0,1/))) then
 call pes_x3y1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y1u1', pes_x3y1u1_pc, pes_x3y1u1_cf)
else if (all(nkj.eq.(/2,2,0,1/))) then
 call pes_x2y2u1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2u1', pes_x2y2u1_pc, pes_x2y2u1_cf)
else if (all(nkj.eq.(/3,0,1,1/))) then
 call pes_x3z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x3z1u1', pes_x3z1u1_pc, pes_x3z1u1_cf)
else if (all(nkj.eq.(/2,1,1,1/))) then
 call pes_x2y1z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y1z1u1', pes_x2y1z1u1_pc, pes_x2y1z1u1_cf)
else if (all(nkj.eq.(/1,2,1,1/))) then
 call pes_x1y2z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y2z1u1', pes_x1y2z1u1_pc, pes_x1y2z1u1_cf)
else if (all(nkj.eq.(/4,2,0,0/))) then
...
else if (all(nkj.eq.(/3,2,1,0/))) then
 call pes_x3y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2z1', pes_x3y2z1_pc, pes_x3y2z1_cf)
else if (all(nkj.eq.(/4,1,0,1/))) then
 call pes_x4y1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x4y1u1', pes_x4y1u1_pc, pes_x4y1u1_cf)
else if (all(nkj.eq.(/3,2,0,1/))) then
 call pes_x3y2u1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2u1', pes_x3y2u1_pc, pes_x3y2u1_cf)
else if (all(nkj.eq.(/4,0,1,1/))) then
 call pes_x4z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x4z1u1', pes_x4z1u1_pc, pes_x4z1u1_cf)
else if (all(nkj.eq.(/3,1,1,1/))) then
 call pes_x3y1z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y1z1u1', pes_x3y1z1u1_pc, pes_x3y1z1u1_cf)
else if (all(nkj.eq.(/2,2,1,1/))) then
 call pes_x2y2z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2z1u1', pes_x2y2z1u1_pc, pes_x2y2z1u1_cf)
else if (all(nkj.eq.(/4,2,1,0/))) then
 call pes_x4y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x4y2z1', pes_x4y2z1_pc, pes_x4y2z1_cf)
else if (all(nkj.eq.(/4,2,0,1/))) then
 call pes_x4y2u1_add (pc, cf)
 call pes_write (iun, 'pcf-x4y2u1', pes_x4y2u1_pc, pes_x4y2u1_cf)
else if (all(nkj.eq.(/4,1,1,1/))) then
 call pes_x4y1z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x4y1z1u1', pes_x4y1z1u1_pc, pes_x4y1z1u1_cf)
else if (all(nkj.eq.(/3,2,1,1/))) then
 call pes_x3y2z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2z1u1', pes_x3y2z1u1_pc, pes_x3y2z1u1_cf)
else if (all(nkj.eq.(/4,2,1,1/))) then
 call pes_x4y2z1u1_add (pc, cf)
 call pes_write (iun, 'pcf-x4y2z1u1', pes_x4y2z1u1_pc, pes_x4y2z1u1_cf)
else
 stop 'pes_x4y2z1u1_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x4y2z1u1_getcf
