SUBROUTINE pes_y1z1u1_getcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_y1z1u1_nki)) then
 stop 'pes_y1z1u1_getcf: bad dimension'
else if (any(nki.ne.pes_y1z1u1_nki)) then
 stop 'pes_y1z1u1_getcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_write (iun, 'pcf-y1z1u1', &
   pes_y1z1u1_pc, pes_y1z1u1_cf)
else
 stop 'pes_y1z1u1_getcf: bad nkj'
end if
return
END SUBROUTINE pes_y1z1u1_getcf
