SUBROUTINE pes_x4y4z1_getvcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x4y4z1_nki)) then
 stop 'pes_x4y4z1_getvcf: bad dimension'
else if (any(nki.ne.pes_x4y4z1_nki)) then
 stop 'pes_x4y4z1_getvcf: bad nki'
endif
...
else if (all(nkj.eq.(/4,4,1/))) then
 call pes_x4y4z1_vadd (pc, cf)
 call pes_write (iun, 'vpcf-x4y4z1', &
   pes_x4y4z1_vpc, pes_x4y4z1_vcf)
else
 stop 'pes_x4y4z1_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x4y4z1_getvcf
