SUBROUTINE pes_x3y2_getvcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x3y2_nki)) then
 stop 'pes_x3y2_getvcf: bad dimension'
else if (any(nki.ne.pes_x3y2_nki)) then
 stop 'pes_x3y2_getvcf: bad nki'
endif
if (all(nkj.eq.(/1,0/))) then
 ...
 stop 'pes_x3y2_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x3y2_getvcf
