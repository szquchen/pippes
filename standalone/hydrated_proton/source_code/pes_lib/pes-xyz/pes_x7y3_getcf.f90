SUBROUTINE pes_x7y3_getcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x7y3_nki)) then
 stop 'pes_x7y3_getcf: bad dimension'
else if (any(nki.ne.pes_x7y3_nki)) then
 stop 'pes_x7y3_getcf: bad nki'
endif
if (all(nkj.eq.(/1,0/))) then
 ...
 stop 'pes_x7y3_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x7y3_getcf
