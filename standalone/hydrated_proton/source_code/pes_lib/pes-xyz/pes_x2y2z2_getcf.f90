SUBROUTINE pes_x2y2z2_getcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x2y2z2_nki)) then
 stop 'pes_x2y2z2_getcf: bad dimension'
else if (any(nki.ne.pes_x2y2z2_nki)) then
 stop 'pes_x2y2z2_getcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_write (iun, 'pcf-x2z2', &
   pes_x2z2_pc, pes_x2z2_cf)
 ...
 call pes_write (iun, 'pcf-y2z2', &
   pes_y2z2_pc, pes_y2z2_cf)
 ...
 stop 'pes_x2y2z2_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x2y2z2_getcf
