FUNCTION pes_x2y3_vfun (xn) RESULT (f)
! Vector function for generic X2Y3
real (kind=wp), intent (in) :: xn(0:,0:)
real (kind=wp) :: f(0:size(xn,2)-1)
!-----------------------------------------------------------------------
real (kind=wp) :: r(0:pes_x2y3_nk-1,0:pes_x2y3_nk-1)
integer, parameter :: nki(0:1)=pes_x2y3_nki
call pes_dists (xn, r)
