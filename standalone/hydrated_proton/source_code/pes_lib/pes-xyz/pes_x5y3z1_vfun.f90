FUNCTION pes_x5y3z1_vfun (xn) RESULT (f)
! Vector function for generic X5Y3Z1
real (kind=wp), intent (in) :: xn(0:,0:)
real (kind=wp) :: f(0:size(xn,2)-1)
!-----------------------------------------------------------------------
real (kind=wp) :: r(0:pes_x5y3z1_nk-1,0:pes_x5y3z1_nk-1)
integer, parameter :: nki(0:2)=pes_x5y3z1_nki
call pes_dists (xn, r)
f = cxv_f1(nki,pes_x1_vcf)+ &
  ...
  cxv_f531(nki,r,pes_x5y3z1_vpc,pes_x5y3z1_vcf)
return
END FUNCTION pes_x5y3z1_vfun
