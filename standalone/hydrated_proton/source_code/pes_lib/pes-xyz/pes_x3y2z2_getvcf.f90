SUBROUTINE pes_x3y2z2_getvcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x3y2z2_nki)) then
 stop 'pes_x3y2z2_getvcf: bad dimension'
else if (any(nki.ne.pes_x3y2z2_nki)) then
 stop 'pes_x3y2z2_getvcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_write (iun, 'vpcf-x2y2', &
   pes_x2y2_vpc, pes_x2y2_vcf)
 ...
 call pes_write (iun, 'vpcf-x1y2z1', &
   pes_x1y2z1_vpc, pes_x1y2z1_vcf)
 ...
 call pes_write (iun, 'vpcf-x3y2', &
   pes_x3y2_vpc, pes_x3y2_vcf)
 ...
 call pes_write (iun, 'vpcf-x2y2z1', &
   pes_x2y2z1_vpc, pes_x2y2z1_vcf)
 ...
 call pes_write (iun, 'vpcf-x1y2z2', &
   pes_x1y2z2_vpc, pes_x1y2z2_vcf)
else if (all(nkj.eq.(/3,2,1/))) then
 call pes_x3y2z1_vadd (pc, cf)
 call pes_write (iun, 'vpcf-x3y2z1', &
   pes_x3y2z1_vpc, pes_x3y2z1_vcf)
 ...
 call pes_write (iun, 'vpcf-x2y2z2', &
   pes_x2y2z2_vpc, pes_x2y2z2_vcf)
else if (all(nkj.eq.(/3,2,2/))) then
 call pes_x3y2z2_vadd (pc, cf)
 call pes_write (iun, 'vpcf-x3y2z2', &
   pes_x3y2z2_vpc, pes_x3y2z2_vcf)
else
 stop 'pes_x3y2z2_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x3y2z2_getvcf
