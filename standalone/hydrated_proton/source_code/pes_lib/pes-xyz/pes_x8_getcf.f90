SUBROUTINE pes_x8_getcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x8_nki)) then
 stop 'pes_x8_getcf: bad dimension'
else if (any(nki.ne.pes_x8_nki)) then
 stop 'pes_x8_getcf: bad nki'
endif
if (all(nkj.eq.(/1/))) then
 ...
 call pes_x8_add (pc, cf)
 call pes_write (iun, 'pcf-x8', pes_x8_pc, pes_x8_cf)
else
 stop 'pes_x8_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x8_getcf
