SUBROUTINE pes_x2y2z2_getvcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x2y2z2_nki)) then
 stop 'pes_x2y2z2_getvcf: bad dimension'
else if (any(nki.ne.pes_x2y2z2_nki)) then
 stop 'pes_x2y2z2_getvcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_write (iun, 'vpcf-y2z2', &
   pes_y2z2_vpc, pes_y2z2_vcf)
 ...
 stop 'pes_x2y2z2_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x2y2z2_getvcf
