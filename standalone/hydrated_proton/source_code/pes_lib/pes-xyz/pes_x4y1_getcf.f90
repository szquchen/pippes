SUBROUTINE pes_x4y1_getcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x4y1_nki)) then
 stop 'pes_x4y1_getcf: bad dimension'
else if (any(nki.ne.pes_x4y1_nki)) then
 stop 'pes_x4y1_getcf: bad nki'
endif
if (all(nkj.eq.(/1,0/))) then
 ...
 stop 'pes_x4y1_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x4y1_getcf
