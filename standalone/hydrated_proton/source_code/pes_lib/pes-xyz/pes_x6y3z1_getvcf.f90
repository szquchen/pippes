SUBROUTINE pes_x6y3z1_getvcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x6y3z1_nki)) then
 stop 'pes_x6y3z1_getvcf: bad dimension'
else if (any(nki.ne.pes_x6y3z1_nki)) then
 stop 'pes_x6y3z1_getvcf: bad nki'
endif
...
else if (all(nkj.eq.(/6,3,1/))) then
 call pes_x6y3z1_vadd (pc, cf)
 call pes_write (iun, 'vpcf-x6y3z1', pes_x6y3z1_vpc, pes_x6y3z1_vcf)
else
 stop 'pes_x6y3z1_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x6y3z1_getvcf
