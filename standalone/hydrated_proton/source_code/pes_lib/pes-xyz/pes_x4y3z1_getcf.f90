SUBROUTINE pes_x4y3z1_getcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x4y3z1_nki)) then
 stop 'pes_x4y3z1_getcf: bad dimension'
else if (any(nki.ne.pes_x4y3z1_nki)) then
 stop 'pes_x4y3z1_getcf: bad nki'
endif
...
else if (all(nkj.eq.(/4,3,1/))) then
 call pes_x4y3z1_add (pc, cf)
 call pes_write (iun, 'pcf-x4y3z1', pes_x4y3z1_pc, pes_x4y3z1_cf)
else
 stop 'pes_x4y3z1_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x4y3z1_getcf
