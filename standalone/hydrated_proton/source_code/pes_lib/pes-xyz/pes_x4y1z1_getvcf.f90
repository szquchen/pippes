SUBROUTINE pes_x4y1z1_getvcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x4y1z1_nki)) then
 stop 'pes_x4y1z1_getvcf: bad dimension'
else if (any(nki.ne.pes_x4y1z1_nki)) then
 stop 'pes_x4y1z1_getvcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 stop 'pes_x4y1z1_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x4y1z1_getvcf
