SUBROUTINE pes_x5y3z1_getcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x5y3z1_nki)) then
 stop 'pes_x5y3z1_getcf: bad dimension'
else if (any(nki.ne.pes_x5y3z1_nki)) then
 stop 'pes_x5y3z1_getcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_x5y3z1_add (pc, cf)
 call pes_write (iun, 'pcf-x5y3z1', pes_x5y3z1_pc, pes_x5y3z1_cf)
else
 stop 'pes_x5y3z1_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x5y3z1_getcf
