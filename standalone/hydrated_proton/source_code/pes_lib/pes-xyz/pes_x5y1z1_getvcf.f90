SUBROUTINE pes_x5y1z1_getvcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x5y1z1_nki)) then
 stop 'pes_x5y1z1_getvcf: bad dimension'
else if (any(nki.ne.pes_x5y1z1_nki)) then
 stop 'pes_x5y1z1_getvcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_write (iun, 'vpcf-x4y1z1', &
   pes_x4y1z1_vpc, pes_x4y1z1_vcf)
else if (all(nkj.eq.(/5,1,1/))) then
 call pes_x5y1z1_vadd (pc, cf)
 call pes_write (iun, 'vpcf-x5y1z1', &
   pes_x5y1z1_vpc, pes_x5y1z1_vcf)
else
 stop 'pes_x5y1z1_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x5y1z1_getvcf
