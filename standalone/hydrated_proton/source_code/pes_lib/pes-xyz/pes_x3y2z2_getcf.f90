SUBROUTINE pes_x3y2z2_getcf (nki, nkj, pc, cf)
...
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x3y2z2_nki)) then
 stop 'pes_x3y2z2_getcf: bad dimension'
else if (any(nki.ne.pes_x3y2z2_nki)) then
 stop 'pes_x3y2z2_getcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_write (iun, 'pcf-x3y1', &
   pes_x3y1_pc, pes_x3y1_cf)
else if (all(nkj.eq.(/2,2,0/))) then
 call pes_x2y2_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2', &
   pes_x2y2_pc, pes_x2y2_cf)
else if (all(nkj.eq.(/3,0,1/))) then
 call pes_x3z1_add (pc, cf)
 call pes_write (iun, 'pcf-x3z1', &
   pes_x3z1_pc, pes_x3z1_cf)
 ...
 call pes_write (iun, 'pcf-x1y2z1', &
   pes_x1y2z1_pc, pes_x1y2z1_cf)
 ...
 call pes_write (iun, 'pcf-x1y1z2', &
   pes_x1y1z2_pc, pes_x1y1z2_cf)
 ...
 call pes_write (iun, 'pcf-x3y2', &
   pes_x3y2_pc, pes_x3y2_cf)
 ...
 call pes_write (iun, 'pcf-x2y2z1', &
   pes_x2y2z1_pc, pes_x2y2z1_cf)
else if (all(nkj.eq.(/3,0,2/))) then
 call pes_x3z2_add (pc, cf)
 call pes_write (iun, 'pcf-x3z2', &
   pes_x3z2_pc, pes_x3z2_cf)
else if (all(nkj.eq.(/2,1,2/))) then
 call pes_x2y1z2_add (pc, cf)
 call pes_write (iun, 'pcf-x2y1z2', &
   pes_x2y1z2_pc, pes_x2y1z2_cf)
else if (all(nkj.eq.(/1,2,2/))) then
 call pes_x1y2z2_add (pc, cf)
 call pes_write (iun, 'pcf-x1y2z2', &
   pes_x1y2z2_pc, pes_x1y2z2_cf)
else if (all(nkj.eq.(/3,2,1/))) then
 call pes_x3y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2z1', &
   pes_x3y2z1_pc, pes_x3y2z1_cf)
else if (all(nkj.eq.(/3,1,2/))) then
 call pes_x3y1z2_add (pc, cf)
 call pes_write (iun, 'pcf-x3y1z2', &
   pes_x3y1z2_pc, pes_x3y1z2_cf)
else if (all(nkj.eq.(/2,2,2/))) then
 call pes_x2y2z2_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2z2', &
   pes_x2y2z2_pc, pes_x2y2z2_cf)
else if (all(nkj.eq.(/3,2,2/))) then
 call pes_x3y2z2_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2z2', &
   pes_x3y2z2_pc, pes_x3y2z2_cf)
else
 stop 'pes_x3y2z2_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x3y2z2_getcf
