SUBROUTINE pes_x5y2z1_getvcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x5y2z1_nki)) then
 stop 'pes_x5y2z1_getvcf: bad dimension'
else if (any(nki.ne.pes_x5y2z1_nki)) then
 stop 'pes_x5y2z1_getvcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 ...
 call pes_x5y2z1_vadd (pc, cf)
 call pes_write (iun, 'vpcf-x5y2z1', pes_x5y2z1_vpc, pes_x5y2z1_vcf)
else
 stop 'pes_x5y2z1_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x5y2z1_getvcf
