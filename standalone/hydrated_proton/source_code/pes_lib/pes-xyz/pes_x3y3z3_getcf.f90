SUBROUTINE pes_x3y3z3_getcf (nki, nkj, pc, cf)
! Callback routine for inv/cx_getcf
integer, intent (in) :: nki(0:), nkj(0:)
type (cx_t), intent (in) :: pc
real (kind=wp), intent (in) :: cf(0:)
!-----------------------------------------------------------------------
integer :: iun
call pes_getiun (iun)
if (size(nki).ne.size(pes_x3y3z3_nki)) then
 stop 'pes_x3y3z3_getcf: bad dimension'
else if (any(nki.ne.pes_x3y3z3_nki)) then
 stop 'pes_x3y3z3_getcf: bad nki'
endif
if (all(nkj.eq.(/1,0,0/))) then
 call pes_x1_add (cf)
 call pes_write0 (iun, 'pcf-x1', pes_x1_cf)
else if (all(nkj.eq.(/0,1,0/))) then
 call pes_y1_add (cf)
 call pes_write0 (iun, 'pcf-y1', pes_y1_cf)
else if (all(nkj.eq.(/0,0,1/))) then
 call pes_z1_add (cf)
 call pes_write0 (iun, 'pcf-z1', pes_z1_cf)
else if (all(nkj.eq.(/2,0,0/))) then
 call pes_x2_add (pc, cf)
 call pes_write (iun, 'pcf-x2', pes_x2_pc, pes_x2_cf)
else if (all(nkj.eq.(/1,1,0/))) then
 call pes_x1y1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y1', pes_x1y1_pc, pes_x1y1_cf)
else if (all(nkj.eq.(/0,2,0/))) then
 call pes_y2_add (pc, cf)
 call pes_write (iun, 'pcf-y2', pes_y2_pc, pes_y2_cf)
else if (all(nkj.eq.(/1,0,1/))) then
 call pes_x1z1_add (pc, cf)
 call pes_write (iun, 'pcf-x1z1', pes_x1z1_pc, pes_x1z1_cf)
else if (all(nkj.eq.(/0,1,1/))) then
 call pes_y1z1_add (pc, cf)
 call pes_write (iun, 'pcf-y1z1', pes_y1z1_pc, pes_y1z1_cf)
else if (all(nkj.eq.(/0,0,2/))) then
 call pes_z2_add (pc, cf)
 call pes_write (iun, 'pcf-z2', pes_z2_pc, pes_z2_cf)
else if (all(nkj.eq.(/3,0,0/))) then
 call pes_x3_add (pc, cf)
 call pes_write (iun, 'pcf-x3', pes_x3_pc, pes_x3_cf)
else if (all(nkj.eq.(/2,1,0/))) then
 call pes_x2y1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y1', pes_x2y1_pc, pes_x2y1_cf)
else if (all(nkj.eq.(/1,2,0/))) then
 call pes_x1y2_add (pc, cf)
 call pes_write (iun, 'pcf-x1y2', pes_x1y2_pc, pes_x1y2_cf)
else if (all(nkj.eq.(/0,3,0/))) then
 call pes_y3_add (pc, cf)
 call pes_write (iun, 'pcf-y3', pes_y3_pc, pes_y3_cf)
else if (all(nkj.eq.(/2,0,1/))) then
 call pes_x2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x2z1', pes_x2z1_pc, pes_x2z1_cf)
else if (all(nkj.eq.(/1,1,1/))) then
 call pes_x1y1z1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y1z1', pes_x1y1z1_pc, pes_x1y1z1_cf)
else if (all(nkj.eq.(/0,2,1/))) then
 call pes_y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-y2z1', pes_y2z1_pc, pes_y2z1_cf)
else if (all(nkj.eq.(/1,0,2/))) then
 call pes_x1z2_add (pc, cf)
 call pes_write (iun, 'pcf-x1z2', pes_x1z2_pc, pes_x1z2_cf)
else if (all(nkj.eq.(/0,1,2/))) then
 call pes_y1z2_add (pc, cf)
 call pes_write (iun, 'pcf-y1z2', pes_y1z2_pc, pes_y1z2_cf)
else if (all(nkj.eq.(/0,0,3/))) then
 call pes_z3_add (pc, cf)
 call pes_write (iun, 'pcf-z3', pes_z3_pc, pes_z3_cf)
else if (all(nkj.eq.(/3,1,0/))) then
 call pes_x3y1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y1', pes_x3y1_pc, pes_x3y1_cf)
else if (all(nkj.eq.(/2,2,0/))) then
 call pes_x2y2_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2', pes_x2y2_pc, pes_x2y2_cf)
else if (all(nkj.eq.(/1,3,0/))) then
 call pes_x1y3_add (pc, cf)
 call pes_write (iun, 'pcf-x1y3', pes_x1y3_pc, pes_x1y3_cf)
else if (all(nkj.eq.(/3,0,1/))) then
 call pes_x3z1_add (pc, cf)
 call pes_write (iun, 'pcf-x3z1', pes_x3z1_pc, pes_x3z1_cf)
else if (all(nkj.eq.(/2,1,1/))) then
 call pes_x2y1z1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y1z1', pes_x2y1z1_pc, pes_x2y1z1_cf)
else if (all(nkj.eq.(/1,2,1/))) then
 call pes_x1y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y2z1', pes_x1y2z1_pc, pes_x1y2z1_cf)
else if (all(nkj.eq.(/0,3,1/))) then
 call pes_y3z1_add (pc, cf)
 call pes_write (iun, 'pcf-y3z1', pes_y3z1_pc, pes_y3z1_cf)
else if (all(nkj.eq.(/2,0,2/))) then
 call pes_x2z2_add (pc, cf)
 call pes_write (iun, 'pcf-x2z2', pes_x2z2_pc, pes_x2z2_cf)
else if (all(nkj.eq.(/1,1,2/))) then
 call pes_x1y1z2_add (pc, cf)
 call pes_write (iun, 'pcf-x1y1z2', pes_x1y1z2_pc, pes_x1y1z2_cf)
else if (all(nkj.eq.(/0,2,2/))) then
 call pes_y2z2_add (pc, cf)
 call pes_write (iun, 'pcf-y2z2', pes_y2z2_pc, pes_y2z2_cf)
else if (all(nkj.eq.(/1,0,3/))) then
 call pes_x1z3_add (pc, cf)
 call pes_write (iun, 'pcf-x1z3', pes_x1z3_pc, pes_x1z3_cf)
else if (all(nkj.eq.(/0,1,3/))) then
 call pes_y1z3_add (pc, cf)
 call pes_write (iun, 'pcf-y1z3', pes_y1z3_pc, pes_y1z3_cf)
else if (all(nkj.eq.(/3,2,0/))) then
 call pes_x3y2_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2', pes_x3y2_pc, pes_x3y2_cf)
else if (all(nkj.eq.(/2,3,0/))) then
 call pes_x2y3_add (pc, cf)
 call pes_write (iun, 'pcf-x2y3', pes_x2y3_pc, pes_x2y3_cf)
else if (all(nkj.eq.(/3,1,1/))) then
 call pes_x3y1z1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y1z1', pes_x3y1z1_pc, pes_x3y1z1_cf)
else if (all(nkj.eq.(/2,2,1/))) then
 call pes_x2y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2z1', pes_x2y2z1_pc, pes_x2y2z1_cf)
else if (all(nkj.eq.(/1,3,1/))) then
 call pes_x1y3z1_add (pc, cf)
 call pes_write (iun, 'pcf-x1y3z1', pes_x1y3z1_pc, pes_x1y3z1_cf)
else if (all(nkj.eq.(/3,0,2/))) then
 call pes_x3z2_add (pc, cf)
 call pes_write (iun, 'pcf-x3z2', pes_x3z2_pc, pes_x3z2_cf)
else if (all(nkj.eq.(/2,1,2/))) then
 call pes_x2y1z2_add (pc, cf)
 call pes_write (iun, 'pcf-x2y1z2', pes_x2y1z2_pc, pes_x2y1z2_cf)
else if (all(nkj.eq.(/1,2,2/))) then
 call pes_x1y2z2_add (pc, cf)
 call pes_write (iun, 'pcf-x1y2z2', pes_x1y2z2_pc, pes_x1y2z2_cf)
else if (all(nkj.eq.(/0,3,2/))) then
 call pes_y3z2_add (pc, cf)
 call pes_write (iun, 'pcf-y3z2', pes_y3z2_pc, pes_y3z2_cf)
else if (all(nkj.eq.(/2,0,3/))) then
 call pes_x2z3_add (pc, cf)
 call pes_write (iun, 'pcf-x2z3', pes_x2z3_pc, pes_x2z3_cf)
else if (all(nkj.eq.(/1,1,3/))) then
 call pes_x1y1z3_add (pc, cf)
 call pes_write (iun, 'pcf-x1y1z3', pes_x1y1z3_pc, pes_x1y1z3_cf)
else if (all(nkj.eq.(/0,2,3/))) then
 call pes_y2z3_add (pc, cf)
 call pes_write (iun, 'pcf-y2z3', pes_y2z3_pc, pes_y2z3_cf)
else if (all(nkj.eq.(/3,3,0/))) then
 call pes_x3y3_add (pc, cf)
 call pes_write (iun, 'pcf-x3y3', pes_x3y3_pc, pes_x3y3_cf)
else if (all(nkj.eq.(/3,2,1/))) then
 call pes_x3y2z1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2z1', pes_x3y2z1_pc, pes_x3y2z1_cf)
else if (all(nkj.eq.(/2,3,1/))) then
 call pes_x2y3z1_add (pc, cf)
 call pes_write (iun, 'pcf-x2y3z1', pes_x2y3z1_pc, pes_x2y3z1_cf)
else if (all(nkj.eq.(/3,1,2/))) then
 call pes_x3y1z2_add (pc, cf)
 call pes_write (iun, 'pcf-x3y1z2', pes_x3y1z2_pc, pes_x3y1z2_cf)
else if (all(nkj.eq.(/2,2,2/))) then
 call pes_x2y2z2_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2z2', pes_x2y2z2_pc, pes_x2y2z2_cf)
else if (all(nkj.eq.(/1,3,2/))) then
 call pes_x1y3z2_add (pc, cf)
 call pes_write (iun, 'pcf-x1y3z2', pes_x1y3z2_pc, pes_x1y3z2_cf)
else if (all(nkj.eq.(/3,0,3/))) then
 call pes_x3z3_add (pc, cf)
 call pes_write (iun, 'pcf-x3z3', pes_x3z3_pc, pes_x3z3_cf)
else if (all(nkj.eq.(/2,1,3/))) then
 call pes_x2y1z3_add (pc, cf)
 call pes_write (iun, 'pcf-x2y1z3', pes_x2y1z3_pc, pes_x2y1z3_cf)
else if (all(nkj.eq.(/1,2,3/))) then
 call pes_x1y2z3_add (pc, cf)
 call pes_write (iun, 'pcf-x1y2z3', pes_x1y2z3_pc, pes_x1y2z3_cf)
else if (all(nkj.eq.(/0,3,3/))) then
 call pes_y3z3_add (pc, cf)
 call pes_write (iun, 'pcf-y3z3', pes_y3z3_pc, pes_y3z3_cf)
else if (all(nkj.eq.(/3,3,1/))) then
 call pes_x3y3z1_add (pc, cf)
 call pes_write (iun, 'pcf-x3y3z1', pes_x3y3z1_pc, pes_x3y3z1_cf)
else if (all(nkj.eq.(/3,2,2/))) then
 call pes_x3y2z2_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2z2', pes_x3y2z2_pc, pes_x3y2z2_cf)
else if (all(nkj.eq.(/2,3,2/))) then
 call pes_x2y3z2_add (pc, cf)
 call pes_write (iun, 'pcf-x2y3z2', pes_x2y3z2_pc, pes_x2y3z2_cf)
else if (all(nkj.eq.(/3,1,3/))) then
 call pes_x3y1z3_add (pc, cf)
 call pes_write (iun, 'pcf-x3y1z3', pes_x3y1z3_pc, pes_x3y1z3_cf)
else if (all(nkj.eq.(/2,2,3/))) then
 call pes_x2y2z3_add (pc, cf)
 call pes_write (iun, 'pcf-x2y2z3', pes_x2y2z3_pc, pes_x2y2z3_cf)
else if (all(nkj.eq.(/1,3,3/))) then
 call pes_x1y3z3_add (pc, cf)
 call pes_write (iun, 'pcf-x1y3z3', pes_x1y3z3_pc, pes_x1y3z3_cf)
else if (all(nkj.eq.(/3,3,2/))) then
 call pes_x3y3z2_add (pc, cf)
 call pes_write (iun, 'pcf-x3y3z2', pes_x3y3z2_pc, pes_x3y3z2_cf)
else if (all(nkj.eq.(/3,2,3/))) then
 call pes_x3y2z3_add (pc, cf)
 call pes_write (iun, 'pcf-x3y2z3', pes_x3y2z3_pc, pes_x3y2z3_cf)
else if (all(nkj.eq.(/2,3,3/))) then
 call pes_x2y3z3_add (pc, cf)
 call pes_write (iun, 'pcf-x2y3z3', pes_x2y3z3_pc, pes_x2y3z3_cf)
else if (all(nkj.eq.(/3,3,3/))) then
 call pes_x3y3z3_add (pc, cf)
 call pes_write (iun, 'pcf-x3y3z3', pes_x3y3z3_pc, pes_x3y3z3_cf)
else
 stop 'pes_x3y3z3_getcf: bad nkj'
end if
return
END SUBROUTINE pes_x3y3z3_getcf
