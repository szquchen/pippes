SUBROUTINE pes_x4_getvcf (nki, nkj, pc, cf)
...
if (size(nki).ne.size(pes_x4_nki)) then
 stop 'pes_x4_getvcf: bad dimension'
else if (any(nki.ne.pes_x4_nki)) then
 stop 'pes_x4_getvcf: bad nki'
endif
if (all(nkj.eq.(/1/))) then
 ...
 stop 'pes_x4_getvcf: bad nkj'
end if
return
END SUBROUTINE pes_x4_getvcf
